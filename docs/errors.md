...
| `404` | `ERR_USER_NOTFOUND`        | JWT user lookup failed after token parsing |
...
| `409` | `ERR_BAN_ACTIVE_BAN`       | User already restricted. If you encounter this error, remove the ban first and issue a new one |
| `409` | `ERR_USER_NOTBANNED`       | User don't have active restriction                                                             |

### Role admin routes

| HTTP  | `error`                   | Where it comes from                    |
|-------|---------------------------|----------------------------------------|
| `404` | `ERR_ROLE_NOTFOUND`       | Role lookup/edit/delete target missing |
...
| `404` | `ALBUM_NOTFOUND` | Album delete target missing    |
