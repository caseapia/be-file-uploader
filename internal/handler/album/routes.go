...
func (h *Handler) RegisterPrivateRoutes(router fiber.Router) {
	group := router.Group("/album", middleware.Require([]string{"not_restricted_ban"}))
	...
	action.Delete("/delete/:id",
		middleware.RequirePermission(role.FileUpload),
		h.DeleteAlbum,
	)
	group.Get("/lookup/:id",
		middleware.RequirePermission(role.ViewOwnFiles),
		h.LookupAlbum,
	)
	group.Get("/lookupAll",
		middleware.RequirePermission(role.ManageFiles),
		h.AllAlbums,
	)
}
